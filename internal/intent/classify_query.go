...
// ClassifyQuery 对指定 KB 下的可分类意图节点打分（针对单个用户问题）。
//
// 与 ClassifyChunks 的差异：
//   - 输入是单条用户问题，prompt 用 intentClassifyPromptTemplate；
//   - 返回扁平的 []Candidate（一次问题对应一组候选）。
//
// 执行流程：
...
func (c *Classifier) ClassifyQuery(ctx context.Context, kbID int64, question string, topK int, minScore float64) ([]Candidate, error) {
