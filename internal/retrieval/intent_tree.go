...
// 但 API 调用方和前端页面通常需要树形结构。因此，本文件会把扁平的 IntentNode
// 记录转换成带有 Children 子节点的 IntentNodeTreeVO。
//
...
// IntentNodeTreeVO 是面向 API 响应的意图节点树形结构。
//
// 服务层内部使用扁平的 IntentNode 模型存储意图节点；该结构会保留相同的业务字段，
...
type IntentNodeTreeVO struct {
	...
	Children       []IntentNodeTreeVO `json:"children"`
}

// BuildTree 将扁平的意图节点列表转换成嵌套的意图树。
//
// 父子关系由 IntentNode.ParentID 决定。没有父节点的节点会成为根节点；如果某个节点
// 指向的父节点不在本次输入列表中，该节点也会被提升为根节点。这样即使查询结果不完整，
// 也能保证节点仍然出现在返回结果里，不会被静默丢弃。
func BuildTree(nodes []IntentNode) []IntentNodeTreeVO {
	if len(nodes) == 0 {
		return nil
	}
	voByID := make(map[int64]*IntentNodeTreeVO, len(nodes))
	for i := range nodes {
		vo := toTreeVO(&nodes[i])
	...
	var build func(id int64) IntentNodeTreeVO
	build = func(id int64) IntentNodeTreeVO {
		vo := *voByID[id]
	...
	roots := make([]IntentNodeTreeVO, 0, len(rootIDs))
...
func toTreeVO(n *IntentNode) IntentNodeTreeVO {
	vo := IntentNodeTreeVO{
