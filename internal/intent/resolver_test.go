...
func TestMergeGroup_DedupKeepHighestScore(t *testing.T) {
	...
	subs := []SubQuestionIntent{
		{SubQuestion: "Q1", Candidates: []Candidate{
			{NodeID: 1, Kind: KindKB, PartitionName: "p1", Score: 0.7},
		}},
		{SubQuestion: "Q2", Candidates: []Candidate{
			{NodeID: 1, Kind: KindKB, PartitionName: "p1", Score: 0.9}, // 同 NodeID 高分
			{NodeID: 2, Kind: KindMCP, MCPToolID: "tool_x", Score: 0.6},
		}},
	}
...
func TestMergeGroup_AllSystemOnly_PureSystem(t *testing.T) {
	...
	r := &Resolver{}
	subs := []SubQuestionIntent{
		{SubQuestion: "你好", Candidates: []Candidate{
			{NodeID: 99, Kind: KindSystem, Score: 0.95},
		}},
	}
	g := r.MergeGroup(subs)
...
func TestMergeGroup_AllSystemOnly_MixedKbAndSystem_NotShortCircuit(t *testing.T) {
	...
	subs := []SubQuestionIntent{
		{SubQuestion: "你好", Candidates: []Candidate{
			{NodeID: 99, Kind: KindSystem, Score: 0.9},
		}},
		{SubQuestion: "介绍产品", Candidates: []Candidate{
			{NodeID: 5, Kind: KindKB, PartitionName: "p5", Score: 0.85},
		}},
	}
	g := r.MergeGroup(subs)
...
func TestMergeGroup_AllSystemOnly_MultiSystemCandidates_StillShortCircuits(t *testing.T) {
	...
	r := &Resolver{}
	subs := []SubQuestionIntent{
		{SubQuestion: "你好，介绍一下你自己", Candidates: []Candidate{
			{NodeID: 99, Kind: KindSystem, Score: 0.85}, // 问候
			{NodeID: 98, Kind: KindSystem, Score: 0.80}, // 自我介绍
		}},
	}
	g := r.MergeGroup(subs)
...
func TestMergeGroup_AllSystemOnly_SystemPlusKbInOneSubQuestion_NotShortCircuit(t *testing.T) {
	...
	r := &Resolver{}
	subs := []SubQuestionIntent{
		{SubQuestion: "Q", Candidates: []Candidate{
			{NodeID: 99, Kind: KindSystem, Score: 0.7},
			{NodeID: 5, Kind: KindKB, PartitionName: "p5", Score: 0.6},
		}},
	}
	g := r.MergeGroup(subs)
	assert.False(t, g.AllSystemOnly)
